...
static const DefaultMacro internal_macros[] = {
	...
	{DEFAULT_SCHEMA, "strcmp", {"expr1", "expr2", nullptr}, {{nullptr, nullptr}}, "if(expr1 >= expr2, if(expr1 = expr2, 0, 1), -1)"},
	{DEFAULT_SCHEMA, "substring_index", {"expr1", "sep", "index", nullptr}, {{nullptr, nullptr}}, "if(index != 0, if((index > 0), list_reduce((split(expr1, sep)[:index]), (ret, x)->concat_ws('.', ret, x)), list_reduce((split(expr1, sep)[index:]), (ret, x)->concat_ws('.', ret, x))), '')"},
